...
export default async function AuthorsListPage(props: { searchParams: Promise<SP> }) {
  const sp = await props.searchParams
  const all = await listAuthors()
  const q = (sp.q ?? '').toLowerCase()

  const items = all.filter((a) => {
    if (q && !`${a.name} ${a.title ?? ''}`.toLowerCase().includes(q)) return false
    return true
  })
            ...
            <div className="p-4">
              ...
              {(a.linkedin || a.email) && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {a.linkedin && <Badge variant="muted" className="text-[10px]">LinkedIn</Badge>}
                  {a.email && <Badge variant="muted" className="text-[10px]">{a.email}</Badge>}
                </div>
              )}
            </div>
          </Link>
